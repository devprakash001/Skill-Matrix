...
public class UpdateQuestion extends HttpServlet {
    protected void service(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (rowsUpdated > 0) {
                response.sendRedirect("update.jsp?success=Question updated successfully with Question ID: " + ques_id);
            } else {
                response.sendRedirect("update.jsp?error=Failed to update the question. Please try again.");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
